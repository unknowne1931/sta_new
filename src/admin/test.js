...
function Test() {
    ...
    const handleStart = () => {
        apiAdmin.get(`${process.env.REACT_APP_API_URL}/get/singel/ticket/to/test/${id}`)
            .then(res => {
                if (res.data.data) {
    ...
    const reset_seconds_and_refund = (text, t2, level, cat) => {
        apiAdmin.post(`${process.env.REACT_APP_API_URL}/refund/data/and/add/to/users`, { id: id, text: text, ex_seconds: t2, level, cat })
        .then(res=>{
