...
const Cupon = () => {
    ...
    const GetCupon = () =>{
        try{
            setTimeout(()=>{
                fetch(`${"http://192.168.126.1"}/get/cupon/get/all/datas`)
                .then(res => res.json())
                .then(data =>{
                    if(data.data){
    ...
    const PostS = (e) =>{
        try{
            e.preventDefault();
            setAlert(false)
            axios.post(`${"http://192.168.126.1"}/get/new/cupon/for/neww/cupon`, {title, img, valid, body, type, user})
            .then(res=>{
            ...
            <div className='cart-page-main-cnt-02-sub-cnt-01'>
                ...
                {cupon_data.map((data, i) =>{
                    ...
                    const Dele = () =>{
                        try{
                            setAlert(false)
                            axios.delete(`${"http://192.168.126.1"}/delete/cupon/s/by/id/${data._id}`)
                            .then(res=>{
                                if(res.data.Status === "OK"){
                                    GetCupon()
                                }else{
