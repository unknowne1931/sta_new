...
const Show_ticket = () => {
  ...
  const get_data = () => {
    apiAdmin.get("http://192.168.126.1/get/all/tickets/data/admin")
      .then(res => {
        if (res.data.data) {
  ...
  const add = (id, dat, level, cat) => {
    apiAdmin.post("http://192.168.126.1/refund/data/and/add/to/users", { id, text: dat, ex_seconds: sec[id] || "no", level : level, cat: cat  })
      .then(res => {
