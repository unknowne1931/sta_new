...
const Cart = () => {
    ...
    const GetStarsBala = () =>{
        try{
            setAlert(false)
            api.get(`${"http://kalanirdhari.in"}/get/stars/balance/${user}`)
            .then(res =>{
                if(res.data.data){
    ...
    const GetMyCoins = () =>{
        try{
            setAlert(false)
            api.get(`${"http://kalanirdhari.in"}/get/coins/data/by/id/${user}`)
            .then(res=>{
                if(res.data.data){
    ...
    const GetStoreCoins = () =>{
        try{
            setTimeout(()=>{
                setAlert(false)
                api.get(`${"http://kalanirdhari.in"}/get/coin/data`)
                .then(res=>{
                if(res.data.data){
            ...
            <div className='cart-page-main-cnt-01-sub-cnt-01'>
                ...
                {my_Coins.map((data, i) =>{
                    ...
                    const ClaimCoin = (e) =>{
                        ...
                        try{
                            setAlert(false);
                            api.post(`${"http://kalanirdhari.in"}/claim/reqst/coins/admin`,{user, id : data._id})
                            .then(res=>{
                ...
                {store_coins.map((data, i) =>{
                    ...
                    const BuyCoins = (e) =>{
                        try{
                            setAlert(false);
                            e.preventDefault();
                            api.post(`${"http://kalanirdhari.in"}/get/my/conis/get`, { user, id : data._id })
                            .then(res=>{
                                if(res.data.Status === "OK"){
                                    GetMyCoins()
                                    GetStarsBala()
