...
function Test() {
    ...
    const handleStart = () => {
        apiAdmin.get(`http://192.168.126.1/get/singel/ticket/to/test/${id}`)
            .then(res => {
                if (res.data.data) {
    ...
    const reset_seconds_and_refund = (text, t2, level, cat) => {
        apiAdmin.post("http://192.168.126.1/refund/data/and/add/to/users", { id: id, text: text, ex_seconds: t2, level, cat })
        .then(res=>{
