...
const Start = () => {
  ...
  const checkSavedTimerOrFetch = async () => {
    ...
    if (target && savedId) {
      ...
      if (timeLeft > 0) {
        const response = await api.get(`http://kalanirdhari.in/get/question/no/by/user/name/${user}`);
  ...
  const QuitGame = async (e) => {
    ...
    setAlert(false);
    try {
      const response = await axios.delete(`http://kalanirdhari.in/delete/by/user/id/for/valid/data/${user}`);
      const resData = response.data;
  ...
  const VerifyAnswer = async (answer) => {
    ...
    await removeFromDB('targetSecond');
    try {
      const response = await api.post(`http://kalanirdhari.in/verify/answer/question/number`, {
        answer,
        user,
        id: QData._id,
