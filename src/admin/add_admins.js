...
const Add_admins = () => {
    ...
    const fetch_users_data = () =>{
      try{
        apiAdmin.get(`${process.env.REACT_APP_API_URL}/get/all/total/users/data/from/admins/super`)
        .then(res =>{
          if(res.data.data){
    ...
    const Post_data = (e) =>{
      ...
      setAlert(false)
      try{
        apiAdmin.post(`${process.env.REACT_APP_API_URL}/get/employe/login/data/create/new`,{username, password, email ,language : lang})
        .then(res =>{
