...
const Start_Gm = () => {
  ...
  const start_check = () => {
    fetch(`${process.env.REACT_APP_API_URL}/start/or/no/check`)
      .then(res => res.json())
      .then(data => {
        if (data.status) {
  ...
  const handleClick = (text) => {
    if (start.Status === "on") {
      apiAdmin.post(`${process.env.REACT_APP_API_URL}/start/game/by/click`, { status: "off", text })
        .then(res => {
      ...
      const interval = setInterval(() => {
        setCountdown(prev => {
          if (prev <= 1) {
            ...
            apiAdmin.post(`${process.env.REACT_APP_API_URL}/start/game/by/click`, { status: "on" })
              .then(res => {
                if (res.data.Status === "OK") {
                  start_check();
                } else {
