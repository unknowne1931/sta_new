...
const Show_ticket = () => {
  ...
  const get_data = () =>{
    apiAdmin.get("http://kalanirdhari.in/get/all/tickets/data/admin")
    .then(res=>{
      if(res.data.data){
  ...
  const add = (id, dat) =>{
    apiAdmin.post("http://kalanirdhari.in/refund/data/and/add/to/users", {id, text: dat, ex_seconds: sec[id] || "no"})
    .then(res =>{
