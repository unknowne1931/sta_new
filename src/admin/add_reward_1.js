...
const Add_reward_1 = () => {
  ...
  React.useEffect(() => {
    const fetchRewards = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/get/singel/rewards`);
  ...
  const get_pay = () =>{
    apiAdmin.get(`${process.env.REACT_APP_API_URL}/get/won/module/reward/with/upi/data`)
    .then(res =>{
      if(res.data.data){
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiAdmin.post(`${process.env.REACT_APP_API_URL}/add/singel/rewards`, {
        data: inputValue
      }, {
        headers: {
        ...
        <div className='reward-list-pay-main'>
          {rew_data.map((data, i) =>{
            ...
            const delet_doc_paid = () =>{
              apiAdmin.delete(`${process.env.REACT_APP_API_URL}/delet/paid/after/one/day/data/${data.reward._id}`)
              .then(res=>{
                if(res.data.Status === "OK"){
                  get_pay()
                }
