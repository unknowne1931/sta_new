...
const Home = () => {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      if (cached && (now - cached.timestamp < CACHE_TIME)) {
        ...
        try {
          const response = await fetch("http://kalanirdhari.in/get/total/users/by/winners/datas/all/one");
  ...
  function get_review() {
    axios.get("http://kalanirdhari.in/comment/review/frome/all/users")
      .then(res => {
        if (res.data.data) {
  ...
  //   const res = await api.get(`http://kalanirdhari.in/comment/get/single/data/email/${email}`);
  //   if (res.data.data){
      ...
      <center>
        ...
        <div className='sm_main_1'>
          ...
          {rate_data.map((dat, i) => {
            ...
            // function like() {
            //   api.post("http://kalanirdhari.in/make/like/review/count", { l_id: dat._id, email: data.email })
            //     .then(res => {
            //       if (res.data.Status === "OK") {
            //         get_review()
