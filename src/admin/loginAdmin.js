...
const LoginAdmin = () => {
    ...
    const SendOtp = (e) =>{
        try{
            e.preventDefault()
            axios.post(`${process.env.REACT_APP_API_URL}/login/to/admin/account`,{username})
            .then(res =>{
    ...
    const Login = (e) =>{
        try{
            e.preventDefault()
            axios.post(`${process.env.REACT_APP_API_URL}/verify/otp/and/pass/by/admin`,{username, pass, otp})
            .then(res =>{
                if(res.data.Status === "OK"){
