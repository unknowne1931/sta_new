...
const User_Data = () => {
    ...
    const fetch_Data = (data) => {
        ...
        if (data.length > 0) {
            setSearch(false)
            apiAdmin.get(`http://192.168.126.1/get/all/user/data/new/for/kick/dataa/${data}`)
                .then(res => {
    ...
    const post_new_sec = (e, user, new_bal) => {
        ...
        apiAdmin.post(
            "http://192.168.126.1/add/from/admin/balance/to/balance",
    ...
    const post_new_code = (e, user) => {
        ...
        apiAdmin.post(
            "http://192.168.126.1/verify/data/to-confirm/reported/doc/async",
            { user, id : code }
        )
            .then(res => {
                if (res.data.Status === "OK") {
