...
const Add_reward_1 = () => {
  ...
  React.useEffect(() => {
    const fetchRewards = async () => {
      try {
        const res = await axios.get("http://192.168.126.1/get/singel/rewards");
  ...
  const get_pay = () =>{
    apiAdmin.get("http://192.168.126.1/get/won/module/reward/with/upi/data")
    .then(res =>{
      if(res.data.data){
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiAdmin.post("http://192.168.126.1/add/singel/rewards", {
        data: inputValue
      }, {
        headers: {
        ...
        <div className='reward-list-pay-main'>
          {rew_data.map((data, i) =>{
            ...
            const delet_doc_paid = () =>{
              apiAdmin.delete(`http://192.168.126.1/delet/paid/after/one/day/data/${data.reward._id}`)
              .then(res=>{
                if(res.data.Status === "OK"){
                  get_pay()
                }
