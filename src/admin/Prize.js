...
const Prize = () => {
    ...
    const GetStarsData = () =>{
        try{
            apiAdmin.get(`${"http://192.168.126.1"}/stars/get/all/data/by/stars`)
            .then(res =>{
                if(res.data.data){
    ...
    const RupeePo = (e) =>{
        try{
            e.preventDefault();
            axios.post(`${"http://192.168.126.1"}/rupee/get/for/game`,{rupee})
            .then(res=>{
    ...
    const PostorUpdate = async (e) => {
        try{
            e.preventDefault();
            axios.post(`${"http://192.168.126.1"}/stars/count/one/stars`,{ stars, count: stars_count })
            .then(res =>{
                if(res.data.Status === "OK"){
                    GetStarsData()
                }else if(res.data.Logout === "OUT"){
