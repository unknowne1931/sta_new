...
const Qstallcheck = () => {
  ...
  const GetListsQuest = (e) => {
    try{
      e.preventDefault();
      apiAdmin
        .get(`${"http://192.168.126.1"}/get/admin/all/question/lists/${lang}`)
        .then((res) => {
          if (res.data.data) {
  ...
  const Delet = () =>{
    try{
      axios.delete(`http://192.168.126.1/delete/unwanted/questions/${id}`)
      .then(res =>{
      ...
      <center>
        ...
        <div className="admin_qstallcheck-main-cnt-01">
          {qdata.map((user, i) => {
            ...
            const GetQustNo = (qno) =>{
              try{
                apiAdmin.get(`${"http://192.168.126.1"}/question/one/by/${qno}/${lang}`)
                .then(res => {
        ...
        <div className="admin_qstallcheck-remaining-cnt">
          ...
          {remaining_data.length > 0 ? 
            remaining_data.map((num, i) =>{
              ...
              const GetQustNo = (num) =>{
                try{
                  apiAdmin.get(`${"http://192.168.126.1"}/question/one/by/${num}/${lang}`)
                  .then(res => {
                    if(res.data.question){
                      setShow1(true)
                      setQstData(res.data.question)
